...
const PowerliftingTrainingTab: React.FC<PowerliftingTrainingTabProps> = ({
  ...
  const filteredExercises = useMemo(() => {
    if (!searchQuery.trim()) return powerliftingExercises;
    const query = searchQuery.toLowerCase();
    return powerliftingExercises.filter(ex => 
      ex.name.toLowerCase().includes(query) ||
      ex.description?.toLowerCase().includes(query)
    );
  }, [powerliftingExercises, searchQuery]);
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-600 dark:text-slate-400 mt-1">
            انتخاب حرکت و تنظیم پارامترهای تمرین قدرتی (پاورلیفتینگ)
          </p>
        </div>
        <Button
      ...
      {showExerciseSelector && (
        <Card className="border-2 border-purple-500 shadow-xl">
          <CardHeader>
            <div className="flex items-center justify-between">
              <CardTitle>انتخاب حرکت قدرتی</CardTitle>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => setParams({ ...params, oneRepMaxPercent: parseFloat(e.target.value) || 0 })}
                  min="0"
                  max="100"
                />
              </div>
              ...
              {params.pauseTime !== undefined && (
                <div>
                  <Label htmlFor="pauseTime">زمان مکث (ثانیه)</Label>
                  <Input
                    id="pauseTime"
                    type="number"
                    value={params.pauseTime}
                    onChange={(e) => setParams({ ...params, pauseTime: parseInt(e.target.value) || undefined })}
                    min="0"
                  />
                </div>
              )}
            ...
            <div className="flex items-center gap-4">
              <label className="flex items-center gap-2">
                <input
                  type="checkbox"
                  checked={params.withChains || false}
                  onChange={(e) => setParams({ ...params, withChains: e.target.checked })}
                  className="w-4 h-4"
                />
                <span>با زنجیر</span>
              </label>
              <label className="flex items-center gap-2">
                <input
                  type="checkbox"
                  checked={params.withBands || false}
                  onChange={(e) => setParams({ ...params, withBands: e.target.checked })}
                  className="w-4 h-4"
                />
                <span>با باند</span>
              </label>
      ...
      <CardContent className="p-4">
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1">
            ...
            {(item.parameters.withChains || item.parameters.withBands) && (
              <div className="mt-3 flex gap-2">
                {item.parameters.withChains && <span className="text-xs bg-yellow-100 dark:bg-yellow-900/30 px-2 py-1 rounded">زنجیر</span>}
                {item.parameters.withBands && <span className="text-xs bg-green-100 dark:bg-green-900/30 px-2 py-1 rounded">باند</span>}
              </div>
            )}
