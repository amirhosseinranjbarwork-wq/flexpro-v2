...
import { Exercise, ResistanceParameters } from '../../../types/ultimate-training';
...
const OlympicTrainingTab: React.FC<OlympicTrainingTabProps> = ({
  ...
  const filteredExercises = useMemo(() => {
    if (!searchQuery.trim()) return olympicExercises;
    const query = searchQuery.toLowerCase();
    return olympicExercises.filter(ex => 
      ex.name.toLowerCase().includes(query) ||
      ex.description?.toLowerCase().includes(query)
    );
  }, [olympicExercises, searchQuery]);
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-600 dark:text-slate-400 mt-1">
            انتخاب حرکت و تنظیم پارامترهای تمرین المپیک (وزنه‌برداری)
          </p>
        </div>
        <Button
      ...
      {showExerciseSelector && (
        <Card className="border-2 border-amber-500 shadow-xl">
          <CardHeader>
            <div className="flex items-center justify-between">
              <CardTitle>انتخاب حرکت المپیک</CardTitle>
          ...
          <CardContent>
            ...
            <div className="mb-4">
              <div className="relative">
                ...
                <Input
                  placeholder="جستجوی حرکت المپیک..."
                  value={searchQuery}
          ...
          <div className="space-y-3">
            {exercises.length === 0 ? (
              <Card className="p-12 text-center">
                ...
                <Button
                  onClick={() => setShowExerciseSelector(true)}
                  className="mt-4 bg-gradient-to-r from-amber-600 to-yellow-600"
                >
                  <Plus className="w-4 h-4 ml-2" />
                  افزودن اولین حرکت
                </Button>
