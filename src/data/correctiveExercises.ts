/**
 * Corrective Exercises Database
 * Converted from data for develop/data/correctiveExercises.js
 */

import { Exercise } from './exercises';

export const correctiveExercisesData: Record<string, string[]> = {
  "کایفوز و سر به جلو (Kyphosis/Forward Head)": [
    "کشش سینه در چارچوب در (Doorway Stretch)",
    "کشش سینه با کش (Band Chest Stretch)",
    "کشش سینه با دیوار (Wall Chest Stretch)",
    "حرکت YTWL روی زمین",
    "حرکت YTWL با کش",
    "حرکت YTWL با دمبل سبک",
    "وال انجل (Wall Angel)",
    "وال انجل با کش",
    "وال انجل با دمبل سبک",
    "فوم رول سینه ای (Thoracic Foam Rolling)",
    "فوم رول گردن",
    "فوم رول شانه",
    "چانه به داخل (Chin Tuck)",
    "چانه به داخل با کش",
    "چانه به داخل با دمبل سبک",
    "تقویت میان‌کتفی (Scapular Retraction)",
    "تقویت میان‌کتفی با کش",
    "تقویت میان‌کتفی با دمبل",
    "کشش SCM/Levator Scapulae",
    "کشش گردن جانبی",
    "کشش گردن رو به جلو",
    "کشش گردن رو به عقب",
    "تقویت عضلات عمقی گردن",
    "تمرینات تعادلی سر"
  ],
  "لوردوز و گودی کمر (Lordosis)": [
    "کشش خم کننده های ران (Hip Flexor Stretch)",
    "کشش خم کننده های ران با کش",
    "کشش خم کننده های ران با دیوار",
    "کشش خم کننده های ران با نیمکت",
    "ددباگ (Dead Bug)",
    "ددباگ با کش",
    "ددباگ با دمبل سبک",
    "حرکت گربه-گاو (Cat-Cow)",
    "حرکت گربه-گاو با کش",
    "حرکت گربه-گاو با دمبل سبک",
    "پلانک با تیلت لگن (Plank with Pelvic Tilt)",
    "پلانک با تیلت لگن با کش",
    "پلانک با تیلت لگن با دمبل سبک",
    "تقویت عضلات شکم (Core Strengthening)",
    "تقویت عضلات شکم با کش",
    "تقویت عضلات شکم با دمبل",
    "کشش QL",
    "کشش QL با کش",
    "کشش QL با دیوار",
    "نیلینگ هیپ فلکسور",
    "نیلینگ هیپ فلکسور با کش",
    "نیلینگ هیپ فلکسور با دمبل سبک",
    "کشش همسترینگ",
    "کشش همسترینگ با کش",
    "کشش همسترینگ با دیوار",
    "تقویت عضلات باسن",
    "تقویت عضلات باسن با کش",
    "تقویت عضلات باسن با دمبل"
  ],
  "اسکولیوز (Scoliosis)": [
    "کشش یکطرفه (Unilateral Stretching)",
    "کشش یکطرفه با کش",
    "کشش یکطرفه با دیوار",
    "تمرینات تعادلی (Balance Exercises)",
    "تمرینات تعادلی با توپ",
    "تمرینات تعادلی با کش",
    "پلانک جانبی (Side Plank)",
    "پلانک جانبی با کش",
    "پلانک جانبی با دمبل سبک",
    "تنفس لترال اصلاحی",
    "تنفس لترال با کش",
    "تنفس لترال با دیوار",
    "تقویت عضلات دورکننده/نزدیک‌کننده لگن",
    "تقویت عضلات دورکننده/نزدیک‌کننده لگن با کش",
    "تقویت عضلات دورکننده/نزدیک‌کننده لگن با دمبل",
    "کشش قفسه سینه یکطرفه",
    "کشش قفسه سینه یکطرفه با کش",
    "کشش قفسه سینه یکطرفه با دیوار",
    "تقویت عضلات کمر یکطرفه",
    "تقویت عضلات کمر یکطرفه با کش",
    "تقویت عضلات کمر یکطرفه با دمبل"
  ],
  "زانوی پرانتزی/ضربدری (Knee Valgus/Varus)": [
    "کلامشل (Clamshell)",
    "کلامشل با کش",
    "کلامشل با دمبل سبک",
    "تقویت دورکننده ران (Hip Abduction)",
    "تقویت دورکننده ران با کش",
    "تقویت دورکننده ران با دمبل",
    "استپ داون (Step Down)",
    "استپ داون با کش",
    "استپ داون با دمبل سبک",
    "کشش نزدیک‌کننده (Adductor Stretch)",
    "کشش نزدیک‌کننده با کش",
    "کشش نزدیک‌کننده با دیوار",
    "X-band Walks",
    "X-band Walks با کش",
    "X-band Walks با دمبل سبک",
    "تقویت گلوت مید",
    "تقویت گلوت مید با کش",
    "تقویت گلوت مید با دمبل",
    "کشش IT Band",
    "کشش IT Band با فوم رول",
    "کشش IT Band با کش",
    "تقویت عضلات داخلی ران",
    "تقویت عضلات داخلی ران با کش",
    "تقویت عضلات داخلی ران با دمبل"
  ],
  "پای صاف (Flat Feet)": [
    "کشش و تقویت کف پا",
    "کشش و تقویت کف پا با توپ",
    "کشش و تقویت کف پا با کش",
    "تمرینات تعادل قوس",
    "تمرینات تعادل قوس با توپ",
    "تمرینات تعادل قوس با کش",
    "Calf Raise با تمرکز قوس",
    "Calf Raise با تمرکز قوس با کش",
    "Calf Raise با تمرکز قوس با دمبل سبک",
    "Short Foot Exercise",
    "Short Foot Exercise با کش",
    "Short Foot Exercise با توپ",
    "کشش پلانتار فاشیا",
    "کشش پلانتار فاشیا با توپ",
    "کشش پلانتار فاشیا با کش",
    "تقویت عضلات داخلی پا",
    "تقویت عضلات داخلی پا با کش",
    "تقویت عضلات داخلی پا با دمبل سبک",
    "کشش ساق پا",
    "کشش ساق پا با دیوار",
    "کشش ساق پا با کش",
    "تقویت عضلات ساق پا",
    "تقویت عضلات ساق پا با کش",
    "تقویت عضلات ساق پا با دمبل"
  ],
  "شانه گرد (Rounded Shoulders)": [
    "کشش قفسه سینه",
    "کشش قفسه سینه با کش",
    "کشش قفسه سینه با دیوار",
    "تقویت عضلات پشت شانه",
    "تقویت عضلات پشت شانه با کش",
    "تقویت عضلات پشت شانه با دمبل",
    "کشش گردن",
    "کشش گردن با کش",
    "کشش گردن با دیوار",
    "تقویت عضلات عمقی گردن",
    "تقویت عضلات عمقی گردن با کش",
    "تقویت عضلات عمقی گردن با دمبل سبک"
  ],
  "شانه یخ‌زده (Frozen Shoulder)": [
    "کشش شانه",
    "کشش شانه با کش",
    "کشش شانه با دیوار",
    "تقویت عضلات شانه",
    "تقویت عضلات شانه با کش",
    "تقویت عضلات شانه با دمبل سبک",
    "کشش کپسول شانه",
    "کشش کپسول شانه با کش",
    "کشش کپسول شانه با دیوار",
    "تقویت عضلات روتاتور کاف",
    "تقویت عضلات روتاتور کاف با کش",
    "تقویت عضلات روتاتور کاف با دمبل سبک"
  ],
  "سندرم تونل کارپال (Carpal Tunnel)": [
    "کشش مچ دست",
    "کشش مچ دست با کش",
    "کشش مچ دست با دیوار",
    "تقویت عضلات مچ دست",
    "تقویت عضلات مچ دست با کش",
    "تقویت عضلات مچ دست با دمبل سبک",
    "کشش انگشتان",
    "کشش انگشتان با کش",
    "کشش انگشتان با توپ",
    "تقویت عضلات انگشتان",
    "تقویت عضلات انگشتان با کش",
    "تقویت عضلات انگشتان با توپ"
  ],
  "درد پایین کمر (Lower Back Pain)": [
    "کشش کمر",
    "کشش کمر با کش",
    "کشش کمر با دیوار",
    "تقویت عضلات کمر",
    "تقویت عضلات کمر با کش",
    "تقویت عضلات کمر با دمبل سبک",
    "کشش همسترینگ",
    "کشش همسترینگ با کش",
    "کشش همسترینگ با دیوار",
    "تقویت عضلات شکم",
    "تقویت عضلات شکم با کش",
    "تقویت عضلات شکم با دمبل سبک",
    "کشش باسن",
    "کشش باسن با کش",
    "کشش باسن با دیوار",
    "تقویت عضلات باسن",
    "تقویت عضلات باسن با کش",
    "تقویت عضلات باسن با دمبل"
  ]
};

// Convert to Exercise array format
export const correctiveExercises: Exercise[] = [];

Object.entries(correctiveExercisesData).forEach(([condition, exercises]) => {
  exercises.forEach((exerciseName, index) => {
    const id = `corrective_${condition.toLowerCase().replace(/\s+/g, '_').replace(/[()]/g, '')}_${index}`;
    correctiveExercises.push({
      id,
      name: exerciseName,
      nameEn: exerciseName,
      muscleGroup: condition,
      equipment: exerciseName.includes('کش') ? 'کش' : 
                 exerciseName.includes('دمبل') ? 'دمبل' : 
                 exerciseName.includes('توپ') ? 'توپ' : 
                 exerciseName.includes('فوم رول') ? 'فوم رولر' : 
                 'وزن بدن',
      type: 'corrective',
      mechanics: 'corrective',
      difficulty: 'beginner',
      description: `تمرین اصلاحی برای ${condition}`,
      instructions: [
        'وضعیت صحیح را بگیرید',
        'حرکت را به آرامی انجام دهید',
        'نفس را منظم نگه دارید',
        'در صورت درد متوقف شوید'
      ],
      tips: [
        'حرکت را کنترل شده انجام دهید',
        'به سیگنال‌های بدن توجه کنید',
        'زمان کافی برای ریکاوری اختصاص دهید'
      ],
      commonMistakes: [
        'حرکت سریع و ناگهانی',
        'نادیده گرفتن درد',
        'عدم رعایت فرم صحیح'
      ],
      variations: [],
      primaryMuscles: [],
      secondaryMuscles: [],
      contraindications: [condition],
      executionTime: 30
    });
  });
});

export default correctiveExercises;

