...
export default function CoachDashboard() {
  ...
  useEffect(() => {
    const loadCoachCode = async () => {
      if (user?.id) {
        // DEPRECATED: Coach code functionality moved to local API
        // For now, generate a simple code based on user ID
        const code = user.id ? `COACH-${user.id.slice(0, 8).toUpperCase()}` : 'LOCAL-MODE';
